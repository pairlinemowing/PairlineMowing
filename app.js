...
function renderServices(items=DEFAULT_SERVICES) {
  document.getElementById('servicesGrid').innerHTML = items.map((s,i) => `
    <article class="service-card">
      <div class="service-number">
        ${escapeHtml(s.number || String(i+1).padStart(2,'0'))}
      </div>
  ...
  grid.innerHTML = sortedItems.map(g => `
    <figure class="gallery-card">
      <img
        ...
        alt="${escapeHtml(g.caption || 'Pairline Mowing project photo')}"
      >
      <figcaption>
        ${escapeHtml(g.caption || 'Pairline Mowing project')}
      </figcaption>
    </figure>
  `).join('');
}
function renderReviews(items=DEFAULT_REVIEWS) {
  document.getElementById('reviewsGrid').innerHTML = items.map(r => `
    <article class="review-card">
      <div class="stars" aria-label="5 out of 5 stars">★★★★★</div>
      <h3>${escapeHtml(r.name)}</h3>
      ${
        r.title
          ? `<div class="review-source">${escapeHtml(r.title)}</div>`
          : ''
      }
...
async function loadSupabaseContent() {
  ...
  try {
    ...
    if (settings?.service_area_text) {
      document.getElementById('serviceAreaText').textContent =
